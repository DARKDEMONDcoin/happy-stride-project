...
const CUT_FROM = [
  ...
  // «مؤشر الأداء للقياس بعد 48 ساعة: …» و«التوقيت المقترح: …» ملاحظات إدارية للمالك.
  /^\s*[-•*]?\s*\**\s*(?:مؤشر|مؤشرات)\s+(?:الأداء|الاداء|النجاح)[^:：\n]{0,60}[:：]/u,
  /^\s*[-•*]?\s*\**\s*(?:التوقيت|الموعد|وقت\s+النشر)\s+(?:المقترح|الأفضل|الافضل)\s*\**\s*[:：]/u,
  /^\s*[-•*]?\s*\**\s*(?:سبب\s+(?:الاختيار|الزاوية)|لماذا\s+(?:اخترت|هذه))\b/u,
  /^\s*[-•*]?\s*\**\s*(?:الجمهور\s+المستهدف|زاوية\s+المحتوى|نوع\s+المنشور|المنصة)\s*\**\s*[:：]/u,
];
